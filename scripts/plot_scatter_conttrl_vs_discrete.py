...
CONTTRL_CSV = BASE / "cont-trl" / "runs.csv"
OUT_DIR = BASE / "scatterplots" / "cont-trl_vs_discrete"
...
def main():
    ...
    df_cont = pd.read_csv(CONTTRL_CSV)
    ...
    for sid in scenario_ids:
        for proxy in proxies:
            ...
            plt.scatter(cont["run_idx"], cont["score"], color="blue", alpha=0.8, label=f"cont-trl (n={len(cont)})", s=24)

            plt.xlim(-1, 50)
            plt.ylim(0, 1)
            plt.xlabel("Run index (0-49)")
            plt.ylabel("Score")
            plt.title(f"{sid} — {proxy} : Discrete (red) vs cont-trl (blue)")
            plt.legend()
